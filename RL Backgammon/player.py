...
class Player():
    ...
    def q_learner(self, moves):
        ...
        try: action = choice(best_actions)
    ...
    def calculate_reward(self, board):
        ...
        if self.name == "x":
            if board[0] > 0: reward -= 25
            if board[25] < 0: reward += 25
            for space in board[1:25]:
                if space == 1: reward -= 15
                #if space >= 2: reward += 0.5 * board.index(space)
            return reward
        else:
            if board[25] < 0: reward -= 25
            if board[0] > 0: reward += 25
            for space in board[1:25]:
                if space == -1: reward -= 15
                #if space <= -2: reward += 0.5 * (24 - board.index(space))
            return reward
    ...
    def update_q_table(self, board, reward):
        ...
        for next_dice in possible_next_dice:
            ...
            if fm != None: av_max_score += 500
                ...
                try: av_max_score += max([self.consult_q_table(x) for
                                                        x in all_moves])
                except Exception: av_max_score = 0
        av_max_score /= 5
